...
pub async fn scrape_to_csv(api_key: &str, video_input: &str, out_path: Option<PathBuf>) -> Result<PathBuf> {
    let video_id = parse_video_id(video_input)
        .ok_or_else(|| anyhow!("Could not parse a YouTube video ID from input: {}", video_input))?;

    let out_path = out_path.unwrap_or_else(|| PathBuf::from(format!("{}.csv", video_id)));
...
fn write_csv(out_path: &Path, rows: &[CommentRow]) -> Result<()> {
